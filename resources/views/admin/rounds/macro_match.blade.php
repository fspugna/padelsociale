    ...
    <table class="table table-macro-match table-macro-match-{!! $macroMatch->id !!}">
        ...
        <tr style="background-color: lightgreen">                                    
            <td  style="border-top: 0;">
                ...
                <select name="id_team1_{!! $macroMatch->id !!}"  class="form-control">
                    ...
                    @foreach( $round->group->macro_teams as $macroTeam )                                                                               
                    <option value="{!! $macroTeam->macroTeam->id !!}" 
                        @if($macroTeam->macroTeam->id == $macroMatch->id_team1) 
                        selected
                        @endif
                        >{!! $macroTeam->macroTeam->name !!}</option>
                    @endforeach
                </select>                                                                        
            </td>
            ...
            <td style="border-top: 0;">
                ...
                <select name="id_team2_{!! $macroMatch->id !!}"  class="form-control">
                    ...
                    @foreach( $round->group->macro_teams as $macroTeam )                                       
                    <option value="{!! $macroTeam->macroTeam->id !!}"
                        @if($macroTeam->macroTeam->id == $macroMatch->id_team2) 
                        selected
                        @endif
                        >{!! $macroTeam->macroTeam->name !!}</option>
                    @endforeach
        ...
        @endif
            <td colspan="3" style="background: yellow; border-bottom: 4px solid #f4f4f4">                                       
                @php
                    $edit_mode = 1;
                    $edition_type = $round->group->division->tournament->edition->edition_type;
                @endphp                                    
